...
day_result <- function(day,chem_ID){
  ...
  chem_name <- row.names(chem_con)
  Day <- number_conv(day)
  chem_name <- str_glue("Day{Day}_{chem_name}")  
...
```{r}
cza_plot <- function(index,day = c(0,6,12),save_fig = FALSE){
  # long form convert
  long_cov <- pivot_longer(con_vs_day(day,index),cols = -ES_con, names_to = "variable", values_to = "intensity")
  
  fig <- ggplot(long_cov,aes(x = ES_con,y = intensity,color = variable))+
    geom_point()+
    geom_line()
  if (save_fig){
    ggsave(str_glue("./fig/",toString(index),'.png'),fig,limitsize = F,width = 5,height = 3)
  }
  plot(fig)
}
```
